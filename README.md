...
The R code `Data_analysis.R` evaluates the overall treatment effect of HUD funding, along with the treatment effect per program and per project, on the monthly electricity consumption (logged normalized by sqft.) from 2004 to 2019. Regression adjustments include time and group fixed effects (TWFE) as well as staggered difference-in-differences (DiD) approach. The models are implemented to the initial dataset (`ELC_property.csv`) as well as to the datasets after genetic (`genPanel.csv`) and propensity score (`psmPanel.csv`) matching, standard errors are clustered at the property ID level. The script also contains the code for the visualizations of bias reduction after matching prcedures, parallel trends, and comparison of TWFE and staggered DiD estimators.
