...
plot(H_list,mean_std_diff_list,xlab = 'Number of Observations Pruned' , ylab = 'Average Percent Reduction in Std. Mean Diff.') # Figure S3
...
plot(r_list,mean_std_diff_list,xlab='Ratio',ylab='Average Percent Reduction in Std. Mean Diff.') # Figure S2
...
plot(T_list,mean_std_diff_list,xlab = 'Number of Observations Pruned' , ylab = 'Average Percent Reduction in Std. Mean Diff.') # Figure S4
stopCluster(c1)
...
# merge data and create factors
genPanel <- merge(ELC, gen_matched_data, by = 'AddressIndex') #992724
genPanel <- genPanel %>% 
  mutate(NormConsumption = Consumption/size,
